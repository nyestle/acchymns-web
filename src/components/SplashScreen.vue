...
let change_headers = ref<string[]>(["Anpassung", "Für später herunterladen", "Medienplayer"]);
let change_details = ref<string[]>([
  "Gestalte die Startseite nach deinen Wünschen um.",
  "Lade Gesangbücher für die spätere Nutzung ohne Internetverbindung herunter.",
  "Singe mit dem Klavier mit, indem du den Medienplayer verwendest. Derzeit nur bei bestimmten Gesangbüchern verfügbar.",
]);
  ...
  <div class="fill">
    ...
    <div class="bottom-container">
      ...
      <a class="start-button" @click="action">
        <p class="song__title">{{ scroll_index == change_headers.length - 1 ? "Fortfahren" : "Weiter" }}</p>
      </a>
    </div>
  </div>
</template>
