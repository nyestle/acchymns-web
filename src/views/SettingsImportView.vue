...
async function removeImportedURL(book_to_remove: BookDataSummary) {
  book_to_remove.status = Object.keys(public_references).includes(book_to_remove.id)
...
<template>
  <div class="menu">
    <div class="title">
      ...
      <h1 class="title--center">Liederbücher importieren</h1>
    </div>
  </div>
  ...
  <div class="main-content">
    ...
    <h2 v-if="preview_books.length != 0">Verfügbare Liederbücher</h2>
    <div v-if="preview_books.length != 0" class="warning-label-container">
      ...
      <h5 class="warning-label">Die folgenden Gesangbücher erfordern eine Internetverbindung</h5>
    </div>
    <div>
    ...
    <!-- Imported Books -->
    <h2 v-if="imported_books.length != 0">Importioerte Liederbücher</h2>
    ...
    <div>
      <HomeBookBox id="import-book" v-for="book in imported_books" :key="book.id" :src="book.src" :with-link="false">
        <div class="button-array">
          <button
            v-if="(downloadProgress.get(book.id) || 0) < 1 && Capacitor.getPlatform() !== 'web'"
            @click="download(book)"
          >
    ...
    <!-- Downloaded Books -->
    <h2 v-if="downloaded_books.length != 0">Heruntergeladene Liederbücher</h2>
    <div>
